...
class ZoeppritzTensor(torch.Tensor):
...
class SeismicForward(nn.Module):
    def __init__(self, ntraces, layers, wavemat, Theta1, Theta2, Theta3):
        super(SeismicForward).__init__()
        self.ntraces = ntraces
        self.wavemat = wavemat
        self.Theta1 = Theta1
        self.Theta2 = Theta2
        self.Theta3 = Theta3
        self.zoeppritz = ZoeppritzTensor(ntraces, layers)

    def forward(self, vp, vs, rho):
        if self.ntraces != 1:
            # 传入梯度张量进行正演以及反传
            sys1 = self.zoeppritz(vp, vs, rho, self.Theta1, self.wavemat)
            sys2 = self.zoeppritz(vp, vs, rho, self.Theta2, self.wavemat)
            sys3 = self.zoeppritz(vp, vs, rho, self.Theta3, self.wavemat)

            updates = torch.unsqueeze(torch.stack([sys1, sys2, sys3], dim=1), dim=0)  # (1, 3, input_len)
        else:
            # 传入梯度张量进行正演以及反传
            sys1 = self.zoeppritz(vp, vs, rho, self.Theta1, self.wavemat)
            sys2 = self.zoeppritz(vp, vs, rho, self.Theta2, self.wavemat)
            sys3 = self.zoeppritz(vp, vs, rho, self.Theta3, self.wavemat)
            updates = torch.unsqueeze(torch.stack([sys1, sys2, sys3], dim=0), dim=0)
